...
function drawEffectSize(value)
{
    ...
    if(min < 0)
    {
        for(i=0; i<effectSizeInterpretations[type].length; i++)
        {
            ...
            sideBar.append("line")
                    .attr("x1", L + scale(0) - scale(effectSizeInterpretations[type][i]))
                    .attr("y1", T)
                    .attr("x2", L + scale(0) - scale(effectSizeInterpretations[type][i]))
                    .attr("y2", T + effectSizeHeight)
                    .attr("stroke", "black")
                    .attr("display", "none")
                    .attr("class", "effectSizeInterpretationIndicators");
            sideBar.append("text")
                    .attr("x", L + scale(0) - scale(effectSizeInterpretations[type][i]))
                    .attr("y", T - yAxisTickTextOffset)
                    .attr("transform", "rotate (-45 " + (L + scale(0) - scale(effectSizeInterpretations[type][i])) + " " + (T - yAxisTickTextOffset) + ")")
                    .attr("text-anchor", "start")
                    .attr("font-size", scaleForWindowSize(14) + "px")
                    .text(effectSizeInterpretationIndicators[i])
                    .attr("fill", getColour(type, effectSizeInterpretations[type][i]))
